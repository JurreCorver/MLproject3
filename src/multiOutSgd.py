...
def adaMultiOutBoost(base, numBoost, XTrain, yTrain):
    ...
    for i in range(numBoost):
        #         calibCV = CalibratedClassifierCV(sgdBase, cv=10, method='isotonic')
        base.fit(XTrain, yTrain, sample_weight=np.reshape(initWeights, [-1, ]))
...
def adaBoostMultiPredict(boostModels, boostAlpha, boost, XTest, nClasses):
    ...
    #     predictedTemp[:, i] = predictedTemp[:, i] / np.sum(boostAlpha[0:boost - 1])  # normalization
...
numBoost = 1000
multiOutClf = MultiOutputClassifier(sgdBase)
boostEps, boostAlp, boostModel = adaMultiOutBoost(multiOutClf, numBoost,
                                                  trainingFeatures, trainingTargets)
...
# defined output
baseFileName = '../multiOutSgd'
outBoost = range(100, numBoost+1, 100)

for i in range(len(outBoost)):
    multiOutSgdPred = adaBoostMultiPredict(boostModel, boostAlp, outBoost[i], testingFeatures, 3)
    csvMLP3FormatedOutput(baseFileName + str(i)+'.csv', multiOutSgdPred)
